...
# STEP 5 — Chart 2: Carbon intensity line chart vs the dataset's own annual average -> intensity_vs_benchmark.png
# Benchmarked against this building's own annual mean intensity (not an external EGBC figure,
# since no verified EGBC operational carbon-intensity target for offices could be confirmed)
benchmark = df['Intensity_kgCO2_m2'].mean()
plt.figure(figsize=(10, 5))
plt.plot(df['Month'], df['Intensity_kgCO2_m2'], marker='o', label='Actual')
plt.axhline(y=benchmark, color='red', linestyle='--', label='Annual Average')
plt.title('Monthly Carbon Intensity vs Annual Average')
plt.xlabel('Month')
